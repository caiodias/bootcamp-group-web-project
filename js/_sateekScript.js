...
function getData() {
    ...
    request('https://pixabay.com/api/?key=9176852-528521b115002c31dd0c1b967&q=' + encodeURI(query) + '&image_type=photo&total=20&page=1&per_page=10', function (error, response, body) {
...
function loadResults() {
    ...
    for(let i = 0; i < parsedData['hits'].length; i++) {
        $resultItem = "<div class='shadow'>"
                    +   "<div class='imgDiv'>"
                    +       "<img class='image' src='"+ parsedData['hits'][i]['webformatURL'] +"'>"
                    +   "</div></div>";
